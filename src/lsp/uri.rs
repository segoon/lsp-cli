...
pub fn path_to_file_uri(path: &Path) -> Result<String> {
    let absolute = fs::canonicalize(path)
        .map_err(|error| Error::unexpected(format!("failed to resolve {}: {error}", path.display())))?;
    ...
    .map_err(|()| Error::unexpected(format!("failed to build file URI for {}", absolute.display())))?;

    Ok(url.to_string())
}

pub fn file_uri_to_path(uri: &str) -> Result<PathBuf> {
    let url = Url::parse(uri).map_err(|error| Error::lsp(format!("invalid location URI {uri:?}: {error}")))?;
...
pub fn parse_lsp_uri(uri: &str, context: &str) -> Result<Uri> {
    uri.parse()
        .map_err(|error| Error::lsp(format!("invalid {context} URI {uri:?}: {error}")))
}
